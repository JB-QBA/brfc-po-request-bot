...
def send_quote_email(to_emails, subject, body, filename, file_bytes):
    try:
        ...
        print(f"📧 Email sent via SMTP to: {to_emails}")
...
def get_account_tracking_reference(cost_item: str, department: str):
    ...
    item_idx = headers.index("Cost Item")
    tracking_idx = headers.index("Tracking")
    ref_idx = headers.index("Finance Reference")
...
@app.get("/")
async def root():
    return {"message": "P2P 3000 running via SMTP"}
